...
tw_fund:{label:'台灣基⾦',cur:'TWD',color:'#7F77DD',badge:'bfund'},
twd_cash:{label:'台幣現⾦',cur:'TWD',color:'#888780',badge:''},
usd_cash:{label:'美元現⾦',cur:'USD',color:'#1D9E75',badge:'bus'},
crypto:{label:'加密貨幣',cur:'USD',color:'#D85A30',badge:'bcrypto'},
};
const TYPE_GROUPS={
股票:{types:['tw_stock','us_stock','uk_stock'],color:'#378ADD'},
基⾦:{types:['tw_fund'],color:'#7F77DD'},
現⾦:{types:['twd_cash','usd_cash'],color:'#888780'},
加密貨幣:{types:['crypto'],color:'#D85A30'},
};
const DEBT_TYPES={
mortgage:{label:' 房貸'},car:{label:' ⾞貸'},
...
{sym:'2882',name:'國泰⾦控',type:'tw_stock'},
{sym:'2881',name:'富邦⾦控',type:'tw_stock'},
{sym:'0050',name:'元⼤台灣50',type:'tw_stock'},
{sym:'0056',name:'元⼤⾼股息',type:'tw_stock'},
{sym:'006208',name:'富邦台灣50',type:'tw_stock'},
{sym:'00878',name:'國泰永續⾼股息',type:'tw_stock'},
...
const CRYPTO_IDS={BTC:'bitcoin',ETH:'ethereum',SOL:'solana',BNB:'binancecoin',XRP:'ripple',AD
...
if(sw){
if(familyMembers[m]) sw.classList.add('on');
else sw.classList.remove('on');
}
});
}
// API
async function proxyQuote(s){const r=await fetch(API+'?type=quote&symbol='+encodeURIComponent
async function proxySearch(q){const r=await fetch(API+'?type=search&symbol='+encodeURICompone
async function proxyCrypto(ids){const r=await fetch(API+'?type=crypto&ids='+encodeURIComponen
...
for(const a of assets.filter(x=>['tw_stock','tw_fund','us_stock','uk_stock'].includes(x.typ
try{const q=await proxyQuote(toYahooSymbol(a.ticker,a.type));a.prevPrice=q.prevClose;a.pr
...
ca.forEach(a=>{const id=CRYPTO_IDS[a.ticker.toUpperCase()]||a.ticker.toLowerCase();if(d
}catch{ca.forEach(a=>a.priceSource='error');}
}
saveData();
const now=new Date();
document.getElementById('last-upd').textContent='更新於 '+now.getHours()+':'+String(now.getM
...
if(includeDebt){sw.classList.add('on');if(lbl)lbl.textContent='含債務';if(inc)inc.textConten
else{sw.classList.remove('on');if(lbl)lbl.textContent='不含債務';if(inc)inc.textContent='否（
}
...
if(monthlyRate===0) return{total:principal/totalMonths,principal:principal/totalMonths,inte
if(method==='equal'){
const payment=principal*monthlyRate*Math.pow(1+monthlyRate,totalMonths)/(Math.pow(1+month
...
const p=d.remaining||0,r=d.rate||0,m=d.remainMonths||0,g=d.graceRemain||0,method=d.method||
...
const local=SECURITIES.filter(s=>s.sym.toLowerCase().includes(kw)||s.name.toLowerCase().inc
showAC(local);
...
dd.innerHTML=items.map(s=>'<div class="ac-item" onclick="selectTicker(\''+s.sym+'\',\''+s.n
...
}catch{statusEl.textContent='查詢失敗，請⼿動輸入';statusEl.className='price-status price-err'
}
...
const matches=SECURITIES.filter(s=>s.type==='crypto'&&(s.sym.startsWith(q)||s.name.toUpperC
if(!matches.length){dd.style.display='none';return;}
dd.innerHTML=matches.map(s=>'<div class="ac-item" onclick="selectCrypto(\''+s.sym+'\',\''+s
dd.style.display='block';
}
...
if(profile==='family'){
return assets.filter(a=>familyMembers[a.owner]!==false);
}
...
document.getElementById('today-sub').innerHTML='今⽇損益 <span class="'+CLS(todayTWD)+'">'+(
...
const days=chartPeriod==='1M'?Math.min(30,diffDays):chartPeriod==='1Y'?Math.min(365,diffDay
const labels=[],data=[];
...
type:'line',
data:{labels,datasets:[{data,borderColor:'var(--accent)',backgroundColor:'rgba(2,132,199,
options:{responsive:true,maintainAspectRatio:false,
plugins:{legend:{display:false},tooltip:{callbacks:{label:i=>'TWD '+i.raw.toLocaleStrin
scales:{
x:{ticks:{maxTicksLimit:6,color:'var(--text2)'},grid:{display:false},border:{display:
y:{ticks:{callback:v=>(v>=10000?(v/10000).toFixed(0)+'萬':v)+'',color:'var(--text2)',m
}
}
});
}
...
let slices=[];
if(dim==='detail'){slices=all.map(a=>({label:a.ticker||a.note||CATS[a.type].label,value:ass
else{Object.entries(TYPE_GROUPS).forEach(([n,g])=>{const v=all.filter(a=>g.types.includes(a
...
type:'doughnut',
data:{labels:slices.map(s=>s.label),datasets:[{data:slices.map(s=>s.value),backgroundColo
options:{responsive:true,maintainAspectRatio:true,cutout:'68%',plugins:{legend:{display:f
});
document.getElementById('donut-legend').innerHTML=slices.map(s=>'<div class="legend-item"><
}
...
const fxCell=hasFx&&Math.abs(fxPnl)>0?'<div class="'+CLS(fxPnl)+'" style="font-size:12p
if(isCash) return '<tr onclick="openEditModal('+idx+')">'+'<td><div class="ticker-main"
return '<tr onclick="openEditModal('+idx+')">'+'<td><div class="ticker-main">'+(a.ticke
}).join('');
return '<div class="cat-section"><div class="cat-header"><span class="cat-dot" style="bac
}).join('');
}
function renderDebt(){
...
document.getElementById('debt-include-display').textContent=includeDebt?'是（點擊切換）':'否（
...
const graceStr=d.graceRemain>0?'剩 '+d.graceRemain+' ⽉':'無';
const cur=d.currency||'TWD';
const ownerName=d.owner==='me'?names.me:names.wife;
const result=calcMonthlyPayment(remaining,d.rate||0,d.remainMonths||0,d.graceRemain||0,d.
return '<div class="debt-card" onclick="openDebtEditModal('+idx+')">'+'<div class="debt-c
}).join('');
}
function confirmPayment(idx){
const d=debts[idx];if(!d) return;
const result=calcMonthlyPayment(d.remaining||0,d.rate||0,d.remainMonths||0,d.graceRemain||0
const confirm_msg='確認本⽉還款？\n本⾦：'+fmtCur(result.principal,d.currency||'TWD')+'\n利息：
...
['d-name','d-principal','d-rate','d-total-months','d-grace','d-remaining','d-rate-mid','d-r
...
function debtOverlayClick(e){if(e.target===document.getElementById('debt-modal-overlay'))clos
...
document.getElementById('debt-calc-detail').textContent=grace>0?'寬限期中，僅繳利息':'本⾦ '+fm
}
function submitDebt(){
...
if(!rebalRows.length){rebalRows=all.map(a=>({ticker:a.ticker,name:a.name||a.ticker,nowPct:a
else{rebalRows.forEach(r=>{const a=all.find(x=>x.ticker===r.ticker);if(a){r.nowPct=assetVal
document.getElementById('rebal-rows').innerHTML=rebalRows.map((r,i)=>`<div class="rebal-row
}
...
rebalRows.push({ticker,name:found?.name||ticker,nowPct:0,targetPct:0,price:found?.price||0,
...
let html='<div style="font-size:12px;color:var(--text2);margin-bottom:8px">投入 '+fmtTWD(amo
rebalRows.forEach(r=>{
...
html+='<div class="rebal-result-row"><div><div style="font-weight:600">'+r.ticker+'</div>
});
...
if(isCash){
document.getElementById('cash-lbl').textContent='⾦額（'+CATS[a.type].cur+'）';
...
function closeModal(){document.getElementById('modal-overlay').style.display='none';hideAC();
function overlayClick(e){if(e.target===document.getElementById('modal-overlay'))closeModal();
function goStep1(){document.getElementById('step1').style.display='block';document.getElement
function pickType(t){
selType=t;
document.querySelectorAll('.type-btn').forEach(b=>b.classList.remove('selected'));
...
document.getElementById('form-crypto').style.display=isCrypto?'block':'none';
const cat=CATS[t];
if(cat){document.getElementById('funit').textContent=cat.cur;document.getElementById('cash-
['f-ticker','f-name','f-shares','f-cost','f-note','f-cash-note','f-cry-note'].forEach(id=>{
document.getElementById('f-price').value='';document.getElementById('f-cry-price').value=''
document.getElementById('price-status').textContent='';document.getElementById('cry-price-s
if(!document.getElementById('f-date').value) document.getElementById('f-date').value=todayS
if(!document.getElementById('f-cry-date').value) document.getElementById('f-cry-date').valu
updateFeeUI();
}
...
if(t==='none') el.textContent='無⼿續費 買入總額：'+c+' '+Math.round(p).toLocaleString();
else if(t==='pct') el.textContent='買入總額 '+c+' '+Math.round(p).toLocaleString()+' × '+pct
else el.textContent='買入總額 '+c+' '+Math.round(p).toLocaleString()+' + ⼿續費 '+c+' '+manua
}
...
obj={type:selType,amt,note:document.getElementById('f-cash-note').value,owner:document.ge
}else if(isCrypto){
...
const prevPrice=parseFloat(document.getElementById('f-cry-price').dataset.prevClose)||pri
if(!ticker||!qty)return;
obj={type:'crypto',ticker,qty,cost,price,prevPrice,owner:document.getElementById('f-cry-o
}else{
...
obj={type:selType,ticker,name,shares,cost,price:price||cost,prevPrice:prevPrice||price||c
}
...
profile=p;
['me','wife','family'].forEach(id=>document.getElementById('pb-'+id)?.classList.toggle('act
rebalRows=[];renderAll();
}
function setChartPeriod(btn,p){chartPeriod=p;document.querySelectorAll('.pbtn').forEach(b=>b.
function setDim(btn,d){dim=d;document.querySelectorAll('.dbtn').forEach(b=>b.classList.remove
function toggleRename(){const b=document.getElementById('rename-bar');b.style.display=b.style
