...
const CATS = {
  tw_stock: { label: '台股', cur: 'TWD', color: '#378ADD', badge: 'btw', suffix: '.TW' },
  us_stock: { label: '美股', cur: 'USD', color: '#639922', badge: 'bus', suffix: '' },
  uk_stock: { label: '英股', cur: 'GBP', color: '#BA7517', badge: 'buk', suffix: '.L' },
  tw_fund:  { label: '台灣基金', cur: 'TWD', color: '#7F77DD', badge: 'bfund', suffix: '.TW' },
...
async function twStockQuote(ticker) {
  for (const suf of [':TSE', ':TWSE', '.TW']) {
    try { return await finnhubQuote(ticker + suf); } catch {}
  }
  throw new Error('TW fail');
}

async function cryptoQuote(ticker) {
  ...
  return { price, prevClose: price / (1 + chg / 100) };
}

async function fetchFX() {
  const r = await fetch('https://api.frankfurter.app/latest?from=USD&to=TWD,GBP');
  const d = await r.json();
  const usdTWD = d.rates?.TWD || fx.usd.rate;
  const usdGBP = d.rates?.GBP || (fx.usd.rate / fx.gbp.rate);
  const gbpTWD = usdTWD / usdGBP;
  const yesterday = new Date(); yesterday.setDate(yesterday.getDate() - 1);
  try {
    const r2 = await fetch('https://api.frankfurter.app/' + yesterday.toISOString().split('T')[0] + '?from=USD&to=TWD,GBP');
    const d2 = await r2.json();
    const pUsdTWD = d2.rates?.TWD || usdTWD;
    const pGbpTWD = pUsdTWD / (d2.rates?.GBP || usdGBP);
    fx = { usd: { rate: usdTWD, prev: pUsdTWD }, gbp: { rate: gbpTWD, prev: pGbpTWD } };
  } catch { fx = { usd: { rate: usdTWD, prev: usdTWD }, gbp: { rate: gbpTWD, prev: gbpTWD } }; }
}
...
async function fetchAll() {
  ...
  try { await fetchFX(); updateFxBar(); } catch {}
  for (const a of assets.filter(x => ['tw_stock', 'tw_fund'].includes(x.type))) {
    try { const q = await twStockQuote(a.ticker); a.prevPrice = q.prevClose; a.price = q.price; a.priceSource = 'live'; } catch { a.priceSource = 'error'; }
  }
  for (const a of assets.filter(x => x.type === 'uk_stock')) {
    try { const q = await finnhubQuote(a.ticker + ':LSE'); a.prevPrice = q.prevClose; a.price = q.price; a.priceSource = 'live'; } catch { a.priceSource = 'error'; }
  }
  for (const a of assets.filter(x => x.type === 'us_stock')) {
    try { const q = await finnhubQuote(a.ticker); a.prevPrice = q.prevClose; a.price = q.price; a.priceSource = 'live'; } catch { a.priceSource = 'error'; }
  }
  for (const a of assets.filter(x => x.type === 'crypto')) {
    try { const q = await cryptoQuote(a.ticker); a.prevPrice = q.prevClose; a.price = q.price; a.priceSource = 'live'; } catch { a.priceSource = 'error'; }
  }
  saveData();
  const now = new Date();
...
async function selectTicker(sym, name, type) {
  ...
  try {
    ...
    else if (selType === 'uk_stock') q = await finnhubQuote(sym + ':LSE');
    else q = await finnhubQuote(sym);
  ...
  document.getElementById('asset-sections').innerHTML = Object.keys(CATS).map(k => {
    ...
    const rows = items.map(a => {
      ...
      if (isCash) return '<tr onclick="openEditModal(' + idx + ')">' +
        ...
        '<td class="rc"><div style="font-weight:500">' + fmtCur(valN, cat.cur) + '</div>' + (cat.cur !== 'TWD' ? '<div class="mini">' + fmtTWD(valTWD) + '</div>' : '') + '</td>' +
        '</tr>';
      return '<tr onclick="openEditModal(' + idx + ')">' +
        '<td><div style="font-weight:500;font-size:13px">' + (a.ticker || '—') + liveTag + '</div>' +
        '<div class="mini">' + (a.name && a.name !== a.ticker ? a.name + ' · ' : '') + (a.owner === 'me' ? names.me : names.wife) + (a.note ? ' · ' + a.note : '') + '</div></td>' +
        '<td><span class="badge ' + cat.badge + '">' + cat.label + '</span></td>' +
        ...
        '<td class="rc"><div class="' + CLS(todayN) + '" style="font-size:12px">' + (todayN !== 0 ? (todayN >= 0 ? '+' : '') + fmtCur(todayN, cat.cur) : '—') + '</div>' +
        (hasFx && todayN !== 0 ? '<div class="mini">' + (todayTWD >= 0 ? '+' : '') + fmtTWD(todayTWD) + '</div>' : '') + '</td>' +
        ...
        '<td class="rc"><div style="font-weight:500;font-size:12px">' + fmtCur(valN, cat.cur) + '</div>' +
        (cat.cur !== 'TWD' ? '<div class="mini">' + fmtTWD(valTWD) + '</div>' : '') +
        '<div class="mini ' + CLS(unrealN) + '">' + (unrealN !== 0 ? (unrealN >= 0 ? '+' : '') + fmtCur(unrealN, cat.cur) + ' (' + PCT(unrealPct) + ')' : '—') + '</div></td>' +
        '</tr>';
    }).join('');
...
function applyRename() {
  ...
  document.getElementById('lbl-me').textContent = names.me + '的資產';
  document.getElementById('lbl-wife').textContent = names.wife + '的資產';
  ['f-owner', 'f-cash-owner', 'f-cry-owner'].forEach(id => {
    const s = document.getElementById(id); if (s) { s.options[0].text = names.me; s.options[1].text = names.wife; }
  });
