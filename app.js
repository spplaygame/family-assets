...
function renderAll(){
  ...
  renderTrend(all);renderDonut(all,totalTWD);renderSections(all);
}
...
function renderRebal(){
  ...
  if(!rebalRows.length){
    rebalRows=Object.values(tickerMap).map(r=>({
      ticker:r.ticker,name:r.name,nowPct:r.valueTWD/totalTWD*100,
      targetPct:Math.round(r.valueTWD/totalTWD*100),price:r.price,type:r.type
    }));
  }else{
    rebalRows.forEach(r=>{
      if(tickerMap[r.ticker]){
        r.nowPct=tickerMap[r.ticker].valueTWD/totalTWD*100;
        r.price=tickerMap[r.ticker].price||r.price;
      }
    });
  }
