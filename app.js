...
function loadData(){
  try{
    ...
    if(n){
      ...
      document.querySelectorAll('#d-owner option').forEach((o,i)=>{if(i===0)o.text=names.me;if(i===1)o.text=names.wife;});
    }
...
function renderAll(){
  ...
  if(debts.length>0){debtRow.style.display='flex';}else{debtRow.style.display='none';}
...
function renderDebt(){
  ...
  document.getElementById('debt-list').innerHTML=all.map((d)=>{
    ...
    const monthly=getDebtMonthlyTWD(d);
    ...
    return '<div class="debt-card" onclick="openDebtEditModal('+idx+')">'+'<div class="debt-card-top">'+'<div><div class="debt-card-name">'+typeInfo.label+(d.name?' · '+d.name:'')+'</div>'+'<div class="debt-card-sub">'+(d.owner==='me'?names.me:names.wife)+' · 每月 '+(d.payDay||5)+' 日'+(d.note?' · '+d.note:'')+'</div></div>'+'<div><div class="debt-card-amount">'+fmtCur(remaining,cur)+'</div><div class="debt-card-amount-sub">剩餘本金</div></div>'+'</div>'+'<div class="debt-progress-wrap">'+'<div class="debt-progress-label"><span>已還 '+paidPct+'%</span><span>剩 '+remainStr+'</span></div>'+'<div class="debt-progress-bar"><div class="debt-progress-fill" style="width:'+paidPct+'%"></div></div>'+'</div>'+'<div class="debt-detail-grid">'+'<div class="debt-detail-item"><div class="debt-detail-label">年利率</div><div class="debt-detail-val">'+(d.rate||0)+'%</div></div>'+'<div class="debt-detail-item"><div class="debt-detail-label">本月應還</div><div class="debt-detail-val">'+fmtTWD(monthly)+'</div></div>'+'<div class="debt-detail-item"><div class="debt-detail-label">寬限期</div><div class="debt-detail-val">'+graceStr+'</div></div>'+'</div>'+'</div>';
  }).join('');
}
...
function applyRename(){
  ...
  document.getElementById('lbl-wife').textContent=names.wife+'的資產';
  document.getElementById('set-name-me').textContent=names.me+' / '+names.wife;
  ['f-owner','f-cash-owner','f-cry-owner','d-owner'].forEach(id=>{const s=document.getElementById(id);if(s){s.options[0].text=names.me;s.options[1].text=names.wife;}});
