  ...
  document.getElementById('asset-sections').innerHTML=Object.keys(CATS).map(k=>{
    ...
    const rows=items.map(a=>{
      ...
      if(isCash) return '<tr onclick="openEditModal('+idx+')">' +
        '<td><div class="ticker-main">'+(a.note||cat.label)+'</div><div class="ticker-sub">'+(a.owner==='me'?names.me:names.wife)+'</div></td>' +
        '<td><span class="badge '+cat.badge+'">'+cat.label+'</span></td>' +
        '<td class="rc" style="color:var(--text3)">—</td><td class="rc" style="color:var(--text3)">—</td>' +
        '<td class="rc" style="color:var(--text3)">—</td><td class="rc">'+fxCell+'</td>' +
        '<td class="rc"><div style="font-weight:700;font-size:13px">'+fmtCur(valN,cat.cur)+'</div>'+(cat.cur!=='TWD'?'<div class="mini">'+fmtTWD(valTWD)+'</div>':'')+'</td></tr>';
      return '<tr onclick="openEditModal('+idx+')">' +
        '<td><div class="ticker-main">'+(a.ticker||'—')+liveTag+'</div><div class="ticker-sub">'+(a.name&&a.name!==a.ticker?a.name:'')+(a.note?' · '+a.note:'')+'</div></td>' +
        '<td><span class="badge '+cat.badge+'">'+cat.label+'</span></td>' +
        ...
        '<td class="rc"><div style="font-weight:700;font-size:12px">'+fmtCur(valN,cat.cur)+'</div>'+(cat.cur!=='TWD'?'<div class="mini">'+fmtTWD(valTWD)+'</div>':'')+'<div class="mini '+CLS(unrealN)+'">'+(unrealN!==0?(unrealN>=0?'+':'')+fmtCur(unrealN,cat.cur)+' ('+PCT(unrealPct)+')':'—')+'</div></td>' +
        '</tr>';
    }).join('');
    return '<div class="cat-section"><div class="cat-header"><span class="cat-dot" style="background:'+cat.color+'"></span><span class="cat-title">'+cat.label+'</span><span class="cat-sub">'+cat.cur+'</span><span class="cat-total">'+fmtTWD(catTWD)+'</span></div>' +
      ...
      '<th style="width:17%">標的</th><th style="width:10%">類別</th><th style="width:12%;text-align:right">股數／現價</th>' +
      '<th style="width:13%;text-align:right">均價（'+cat.cur+'）</th><th style="width:16%;text-align:right">今日損益（'+cat.cur+'）</th>' +
      '<th style="width:13%;text-align:right">匯率損益</th><th style="width:19%;text-align:right">未實現損益（'+cat.cur+'）</th>' +
...
function renderRebal(){
  ...
  if(!rebalRows.length){
    rebalRows=all.map(a=>({ticker:a.ticker,name:a.name||a.ticker,nowPct:assetValTWD(a)/totalTWD*100,targetPct:Math.round(assetValTWD(a)/totalTWD*100),price:a.price||0,type:a.type}));
  }else{
    rebalRows.forEach(r=>{const a=all.find(x=>x.ticker===r.ticker);if(a){r.nowPct=assetValTWD(a)/totalTWD*100;r.price=a.price||r.price;}});
  }
  document.getElementById('rebal-rows').innerHTML=rebalRows.map((r,i)=>`
    <div class="rebal-row">
      <div style="flex:1;min-width:0">
        <div class="rebal-ticker">${r.ticker}</div>
        <div class="rebal-name">${r.name!==r.ticker?r.name:''}</div>
      </div>
      <div class="rebal-now">${r.nowPct.toFixed(1)}%</div>
      <i class="ti ti-arrow-right rebal-arrow"></i>
      <div class="iuw rebal-target">
        <input type="number" value="${r.targetPct}" min="0" max="100" style="width:65px;text-align:center;padding:6px 8px" oninput="rebalRows[${i}].targetPct=parseFloat(this.value)||0">
        <span class="iunit" style="right:4px">%</span>
      </div>
      <button class="del-btn" onclick="rebalRemoveRow(${i})"><i class="ti ti-trash"></i></button>
    </div>`).join('');
}
...
function rebalAddRow(){
  ...
  document.getElementById('rebal-add-ticker').value='';
  renderRebal();
}
